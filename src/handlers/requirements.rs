...
pub async fn handle_requirements<C: HttpSend>(
    ...
    match command {
        ...
        RequirementCommands::UploadImage(args) => {
            let (org, project) = resolve_context(&args.context, config)?;
            let path = format!("/org/{org}/project/{project}/requirement/{}/imageUrl/{}", args.id, args.file_id);
        ...
        RequirementCommands::UnlinkJira(args) => {
            let (org, project) = resolve_context(&args.context, config)?;
            let path = format!("/org/{org}/project/{project}/requirement/{}/jiraIssues/{}", args.id, args.jira_issue_id);
            let response = client.send(Method::DELETE, &path, &[], None, true).await?;
            print_output(&response, output)?;
        }
        RequirementCommands::Link(args) => {
            let (org, project) = resolve_context(&args.context, config)?;
            let body = load_json_payload(&args.payload)?;
            let path = format!("/org/{org}/project/{project}/requirement/{}/links/{}", args.id, args.link_type);
            let response = client.send(Method::POST, &path, &[], Some(body), true).await?;
            print_output(&response, output)?;
        }
        RequirementCommands::Unlink(args) => {
            let (org, project) = resolve_context(&args.context, config)?;
            let path = format!("/org/{org}/project/{project}/requirement/{}/links/{}/{}", args.id, args.link_type, args.linked_requirement_id);
            let response = client.send(Method::DELETE, &path, &[], None, true).await?;
            print_output(&response, output)?;
        }
        RequirementCommands::UnlinkCrossProject(args) => {
            let (org, project) = resolve_context(&args.context, config)?;
            let path = format!("/org/{org}/project/{project}/requirement/{}/links/{}/cross_project/{}/{}", args.id, args.link_type, args.linked_project, args.linked_requirement_id);
            let response = client.send(Method::DELETE, &path, &[], None, true).await?;
            print_output(&response, output)?;
        }
        ...
        RequirementCommands::RenameCustomFieldOption(args) => {
            let (org, project) = resolve_context(&args.context, config)?;
            let body = load_json_payload(&args.payload)?;
            let path = format!("/org/{org}/project/{project}/requirements/customFields/renameOption");
            let response = client.send(Method::POST, &path, &[], Some(body), true).await?;
            print_output(&response, output)?;
        }
